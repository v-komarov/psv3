...
from	plat.RunSQL	import	LoadPay4
...
def	CityPay2Txt(self):

    ### --- Файл выгрузки данных ---
    textfile = 'cp-out/abonent.txt'
...
#### --- Обработка построчно ---
	...
	file.write(unicode(wstr,"utf-8").encode("koi8-r")+'\n')
...
def	citypay2Base(self,path):
    ...
    ### --- Лог файл ---
    log = 'cp-log/'+fname+'.log'

    ### --- Файл ошибок ---
    err = 'cp-err/'+fname+'.err'
...
### --- Открытие файлов ---	
	...
	### --- Загрузка в базу ---
	result = LoadPay4(db,qarray[0],qarray[1],qarray[2],qarray[3])
	if result != 'OK':
