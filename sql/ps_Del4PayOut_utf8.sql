CREATE FUNCTION ps_Del4PayOut(varchar(24)) RETURNS text
...
-- Сумма общего баланса до удаления --
balans numeric(10,2);
...
SELECT INTO del_sum ps_service_cost FROM ps_pay_out WHERE ps_rec_id=ps_rec_id_;

-- Определение кода абонента ---
SELECT INTO kod_ab ps_abonent_kod FROM ps_pay_out WHERE ps_rec_id=ps_rec_id_;

-- Определение кода сервиса ---
SELECT INTO kod_ser ps_service_kod FROM ps_pay_out WHERE ps_rec_id=ps_rec_id_;
...
UPDATE ps_pay_out SET ps_rec_delete='delete' WHERE ps_rec_id=ps_rec_id_;
