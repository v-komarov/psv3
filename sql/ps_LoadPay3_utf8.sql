CREATE OR REPLACE FUNCTION ps_LoadPay3(varchar(8),varchar(1),timestamp,varchar(10)) RETURNS text
...
IF length(kod_ab)=0 OR length(ser_)=0 OR date_pay IS NULL OR length(sum_money)=0 THEN
	--- Запись в лог ---
	INSERT INTO ps_loadpay3_log(ps_date_pay,ps_abonent_kod,ps_service_kod,ps_sum,ps_lsp,ps_error,ps_ok) VALUES(date_pay,'','',0.00,'','ОБЩАЯ ОШИБКА ДАННЫХ',0);
...
IF money_sum=0 THEN
	--- Запись в лог ---
	INSERT INTO ps_loadpay3_log(ps_date_pay,ps_abonent_kod,ps_service_kod,ps_sum,ps_lsp,ps_error,ps_ok) VALUES(date_pay,'','',0.00,kod_ab,'ОШИБКА ЗНАЧЕНИЯ СУММЫ',0);
	RETURN 'ERROR IN DATA';
END IF;
...
IF x<>1 THEN
	--- Запись в лог ---
	INSERT INTO ps_loadpay3_log(ps_date_pay,ps_abonent_kod,ps_service_kod,ps_sum,ps_lsp,ps_error,ps_ok) VALUES(date_pay,'','',money_sum,kod_ab,'АБОНЕНТ НЕ НАЙДЕН',0);
...
ELSIF ser_='0' THEN
	...
	ELSE
		--- Запись в лог ---
		INSERT INTO ps_loadpay3_log(ps_date_pay,ps_abonent_kod,ps_service_kod,ps_sum,ps_lsp,ps_error,ps_ok) VALUES(date_pay,kod_rec,'',money_sum,kod_ab,'ОШИБКА ТАРИФНОГО ПЛАНА',0);
...
IF a=0 THEN
	--- Запись в лог ---
	INSERT INTO ps_loadpay3_log(ps_date_pay,ps_abonent_kod,ps_service_kod,ps_sum,ps_lsp,ps_error,ps_ok) VALUES(date_pay,kod_rec,'',money_sum,kod_ab,'ОШИБКА УСЛУГИ',0);
...
IF n=0 THEN
	--- Запись в лог ---
	INSERT INTO ps_loadpay3_log(ps_date_pay,ps_abonent_kod,ps_service_kod,ps_sum,ps_lsp,ps_error,ps_ok) VALUES(date_pay,kod_rec,kod_ser,money_sum,kod_ab,'УСЛУГИ НЕТ В ТАРИФНОМ ПЛАНЕ',0);
	RETURN 'ERROR TARIF PLAN';
END IF;
...
IF n!=0 THEN
	--- Запись в лог ---
	INSERT INTO ps_loadpay3_log(ps_date_pay,ps_abonent_kod,ps_service_kod,ps_sum,ps_lsp,ps_error,ps_ok) VALUES(date_pay,kod_rec,kod_ser,money_sum,kod_ab,'ТАКОЙ ПЛАТЁЖ УЖЕ ЕСТЬ',0);
	RETURN 'EXIST';
END IF;
...
--- Запись в лог ---
INSERT INTO ps_loadpay3_log(ps_date_pay,ps_abonent_kod,ps_service_kod,ps_sum,ps_lsp,ps_ok) VALUES(date_pay,kod_rec,kod_ser,money_sum,kod_ab,1);
